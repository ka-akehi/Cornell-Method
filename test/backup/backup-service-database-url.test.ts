...
function createSource(root: string, relativePath: string, contents: string): string {
...
function assertBackupUsesCanonicalSource({
  ...
  envFile,
  backupsDirectory,
}: { root: string; sourcePath: string; databaseUrl?: string; envFile?: string; backupsDirectory: string }): void {
  ...
  try {
    ...
    withEnvironment({ DATABASE_URL: undefined }, () => {
      ...
      (error: unknown) =>
          ...
          (error as { code: string }).code === "configuration_invalid" &&
          !/postgres|DATABASE_URL/.test((error as { message: string }).message),
      );
    });
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
