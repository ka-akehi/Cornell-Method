...
type BackupOptions = { backupsDirectory?: string };
type BackupApplication = {
  listBackupEntries: (options: BackupOptions) => unknown;
  createBackupEntry: (options: BackupOptions) => unknown;
};
type BackupRoute = {
...
const messages = {
  backup_database_unavailable: "バックアップを作成できませんでした。アプリのデータを利用できない状態です。",
  backup_storage_failure: "バックアップを作成できませんでした。バックアップの保存先を利用できない状態です。",
  backup_configuration_invalid: "バックアップを作成できませんでした。アプリのデータを利用できない状態です。",
  backup_unknown_failure: "バックアップに失敗しました。しばらく待ってから再試行してください。",
} as const;

function loadRoute(application: BackupApplication): BackupRoute {
  ...
  vm.runInNewContext(output, {
    ...
    require: (request: string): unknown => {
      ...
      if (request === "@/shared/http") {
        return {
          apiErrorResponse: (body: unknown) =>
            new Response(JSON.stringify(body), {
              status: 500,
              headers: { "content-type": "application/json" },
            }),
          createBackupApiError: (code: keyof typeof messages) => ({
            code,
  ...
  return moduleObject.exports as unknown as BackupRoute;
}

async function withDesktopBackupDirectory(value: string, callback: () => Promise<void>): Promise<void> {
...
test("backup routes forward the packaged desktop directory to the application service", async () => {
  ...
  let listOptions: BackupOptions | undefined;
  let createOptions: BackupOptions | undefined;
