...
test("desktop update check orchestrates provider, selection, and state without leaking details", () => {
  ...
  assert.match(
    main,
    /#\[tauri::command\]\s*async fn manual_update_check\(\s*app: tauri::AppHandle,[\s\S]*?,/,
  );
  ...
  assert.match(
    main,
    /\.invoke_handler\(tauri::generate_handler!\[[\s\S]*manual_update_check,[\s\S]*read_update_state,[\s\S]*verify_pending_update[\s\S]*\]\)/,
  );
  assert.equal(
    (main.match(/generate_handler!\[[\s\S]*manual_update_check,[\s\S]*read_update_state,[\s\S]*verify_pending_update[\s\S]*\]/g) || []).length,
    1,
  );
  assert.match(main, /#\[tauri::command\]\s*async fn read_update_state\([\s\S]*?\)/);
