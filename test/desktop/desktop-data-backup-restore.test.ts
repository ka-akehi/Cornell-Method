...
test("managed restore keeps the generated safety backup internal and restores the selected latest user backup", {
  skip: !sqliteCliAvailable,
}, async () => {
  await withTemporaryHome(async (homeDirectory) => {
    const ready = createReadyFixture(homeDirectory);
    const userAPath = path.join(ready.backupsDirectory, "user-older.sqlite");
    fs.copyFileSync(ready.databasePath, userAPath);
    sqlite(
      userAPath,
      "UPDATE notebooks SET title = 'Backup A', body = 'backup A body' WHERE id = 'markdown-note';",
    );

    const userBPath = path.join(ready.backupsDirectory, "user-latest.sqlite");
    fs.copyFileSync(ready.databasePath, userBPath);
    const userBCanvasDocument = JSON.stringify({
      schemaVersion: 1,
      page: { width: 1200, height: 800, background: "paper" },
      elements: [
        {
          ...
          height: 40,
          rotation: 0,
          style: { fill: "#166534", fontSize: 20 },
          z: 0,
          text: "backup B canvas",
        },
      ],
    });
    sqlite(
      userBPath,
      `UPDATE notebooks
         SET title = 'Backup B', body = 'backup B body'
       WHERE id = 'markdown-note';
       UPDATE cues SET text = 'backup B cue' WHERE id = 'cue-one';
       UPDATE tags SET name = 'Backup B tag' WHERE id = 'tag-one';
       UPDATE notebook_canvases
          SET document_json = '${userBCanvasDocument.replaceAll("'", "''")}',
              search_text = 'backup B canvas'
        WHERE notebook_id = 'canvas-note';`,
    );
    const userBBefore = digest(userBPath);

    const response = runSidecar(
      homeDirectory,
      request(
        "restore",
        { kind: "managed-backup", backupId: "user-latest.sqlite" },
        { operationId: "catalog-restore" },
      ),
    );

    assert.equal(response.ok, true);
    assert.equal(digest(userBPath), userBBefore);

    const catalog = listManagedBackupCatalog({ storagePaths: ready });
    const safetyBackup = catalog.backups.find(
      (entry) => entry.backupId === "restore-catalog-restore.sqlite.bak",
    );
    assert.ok(safetyBackup);
    assert.equal(safetyBackup.recoveryOnly, true);
    assert.equal(fs.existsSync(path.join(ready.backupsDirectory, safetyBackup.backupId)), true);
    const userBackups = catalog.backups.filter((entry) => !entry.recoveryOnly);
    assert.deepEqual(userBackups.map((entry) => entry.backupId), [
      "user-latest.sqlite",
      "user-older.sqlite",
    ]);
    assert.equal(userBackups[0].backupId, "user-latest.sqlite");

    assert.deepEqual(
      queryJson(ready.databasePath, "SELECT id, title, body, body_mode FROM notebooks ORDER BY id"),
      [
        { id: "canvas-note", title: "Canvas Note", body: "", body_mode: "canvas" },
        { id: "markdown-note", title: "Backup B", body: "backup B body", body_mode: "markdown" },
      ],
    );
    assert.deepEqual(
      queryJson(ready.databasePath, "SELECT notebook_id, text, \"order\" FROM cues"),
      [{ notebook_id: "markdown-note", text: "backup B cue", order: 0 }],
    );
    assert.deepEqual(
      queryJson(
        ready.databasePath,
        "SELECT notebook_id, tag_id, \"order\", tags.name FROM notebook_tags JOIN tags ON tags.id = notebook_tags.tag_id",
      ),
      [{ notebook_id: "markdown-note", tag_id: "tag-one", order: 0, name: "Backup B tag" }],
    );
    assert.deepEqual(
      queryJson(ready.databasePath, "SELECT notebook_id, document_json, search_text FROM notebook_canvases"),
      [{
        notebook_id: "canvas-note",
        document_json: userBCanvasDocument,
        search_text: "backup B canvas",
      }],
    );
    assertStagingClean(ready);
