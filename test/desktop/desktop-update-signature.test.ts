...
const fixture = JSON.parse(fixtureText) as SignatureFixture;
...
function normalizeMacOsVersion(value: string) {
...
function canonicalPayload(fixtureValue: SignatureFixture) {
...
function assertNoSecretOrLocatorMaterial(value: string) {
  assert.doesNotMatch(
    value,
...
test("update signature fixture fixes the v1 wire contract", () => {
  ...
  for (const field of ["keyId", "publicKeyHex", "canonicalPayloadHex", "proof", "expected"] as const) {
