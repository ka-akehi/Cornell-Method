...
function catalogEntry(backupId, createdAt = "2026-08-25T00:00:00.000Z") {
  return {
    backupId,
    fileName: backupId,
...
test("managed backup catalog returns allowlisted metadata without reading or changing source bytes", () => {
  withTemporaryHome((homeDirectory) => {
    ...
    assert.deepEqual(
      [...result.backups]
        .map(({ backupId, fileName, size }) => ({ backupId, fileName, size }))
        .sort((left, right) => left.backupId < right.backupId ? -1 : 1),
      [
        { backupId: "second.sqlite", fileName: "second.sqlite", size: "another private backup".length },
        { backupId: "first.sqlite", fileName: "first.sqlite", size: "SQLite bytes stay private".length },
      ].sort((left, right) => left.backupId < right.backupId ? -1 : 1),
    );
    assert.equal(
