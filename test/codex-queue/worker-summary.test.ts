...
function createCodexStub(stubPath: string): void {
  const source = [
    "#!/bin/sh",
    "set -eu",
    "",
    ...
    "    ;;",
    "  fallback)",
    '    call_count="$(wc -l < "$args_log" | tr -d " ")"',
    '    if [ "$call_count" -eq 1 ]; then',
      "      printf '%s\\n' 'STALE_MODEL_ATTEMPT' > \"$last_message\"",
      "      printf '%s\\n' 'ERROR: model reasoning effort not supported' >&2",
      "      exit 1",
    "    fi",
    '    cat "$report_file" >> "$last_message"',
    `    printf '%s\\n' '${rawStdoutMarker}'`,
    ...
    `    printf '%s\\n' '${trackedArtifact}' >> "$WORKER_CHANGED_FILES_FILE"`,
...
function removeHarness(harness: WorkerHarness): void {
  fs.rmSync(harness.root, { recursive: true, force: true });
  fs.rmSync(harness.runtimeTemp, { recursive: true, force: true });
}

function findSummary(root: string): string {
...
function readSummary(root: string): string {
  return fs.readFileSync(findSummary(root), "utf8");
}

function readNonemptyLines(file: string): string[] {
  return fs
...
function runWorker(
  ...
  return new Promise((resolve, reject) => {
    const child: ChildProcessByStdio<null, Readable, Readable> = spawn(path.join(harness.binDir, "worker-run.sh"), [], {
      cwd: harness.root,
...
function assertTemporaryFilesRemoved(harness: WorkerHarness): void {
...
test("excludes runtime artifacts while retaining tracked and untracked task outputs", async () => {
  ...
  try {
    ...
    assert.equal(output.includes(untrackedArtifact), false, untrackedArtifact);
    const changesMade = summary.slice(summary.indexOf("## Changes Made"));
    assert.equal(
      changesMade.includes(
        `| \`${trackedArtifact}\` | Worker が意図的変更として記録 |`,
      ),
      true,
    );
    assert.equal(
      changesMade.includes(`| \`${untrackedArtifact}\` |`),
      false,
    );
...
test("captures the final message for every codex exec route", async () => {
  const cases = [
    ...
    },
    {
      ...
      scenario: "fallback",
      environment: {},
      expectedModels: ["gpt-5.6-luna", "gpt-5.6-luna"],
    },
  ];
