...
function createCheckSummaryStub(destination: string): void {
  fs.writeFileSync(destination, "#!/bin/sh\nset -eu\n[ -f \"$1\" ]\n");
  fs.chmodSync(destination, 0o755);
}

function createCodexStub(destination: string): void {
  fs.writeFileSync(
    destination,
    [
      "#!/bin/sh",
...
function createHarness(name: string, taskBody: string): WorkerHarness {
