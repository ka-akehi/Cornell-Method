...
function readColorToken(foundation: string, tokenName: string): string {
  const match = foundation.match(
    new RegExp(`${tokenName}:\\s*(#[0-9a-fA-F]{6})`),
  );
  assert.ok(match, `Missing color token ${tokenName}`);
  if (!match) throw new Error(`Missing color token ${tokenName}`);
  return match[1];
}

function relativeLuminance(hex: string): number {
  const matchedChannels = hex
    .slice(1)
    .match(/../g);
  if (!matchedChannels) throw new Error(`Invalid color: ${hex}`);
...
function assertNormalTextContrast(label: string, foreground: string, background: string): void {
...
test("filled-control states use the AA-safe token without changing semantic states", () => {
  ...
  assert.match(
    appShell,
    /\.app-chrome-nav-link\.is-selected,[\s\S]*background: var\(--app-active\);[\s\S]*color: var\(--app-accent-deep\);/,
  );
  assert.match(
    appShell,
    /\.app-chrome-create-link:hover,[\s\S]*\.app-chrome-create-link\.is-selected,[\s\S]*background: var\(--app-accent-deep\);[\s\S]*color: var\(--app-accent-contrast\);/,
  );
  assert.match(
    paper,
    /\.note-paper-footer button\[type="submit"\]:not\(:disabled\)[\s\S]*border-color: var\(--app-accent-deep\);[\s\S]*background-color: var\(--app-accent-deep\);[\s\S]*color: var\(--app-accent-contrast\);/,
  );
  assert.match(
    paper,
