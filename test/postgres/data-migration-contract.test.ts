...
function createFixture(): { databasePath: string; directory: string } {
...
test("better-sqlite3 native load failure falls back to the sqlite3 CLI", {
  ...
  const nativeLoadError = Object.assign(new Error("native module load failed"), {
...
test("better-sqlite3 constructor native load failure falls back to the sqlite3 CLI", {
  ...
  const fixture = createFixture();
  const originalLoad = moduleLoader._load;
  const nativeLoadError = Object.assign(new Error("native module load failed in constructor"), {
    code: "ERR_DLOPEN_FAILED",
  });
...
test("SQLite CANTOPEN failure does not fall back after better-sqlite3 loads", () => {
  ...
  const originalLoad = moduleLoader._load;
  const openError = Object.assign(new Error("database open failed"), {
...
test("SQLite query failure does not fall back after better-sqlite3 loads", () => {
  ...
  const originalLoad = moduleLoader._load;
  const queryError = Object.assign(new Error("database query failed"), {
    code: "SQLITE_NOTADB",
  });
  class FakeDatabase {
    pragma() {}
...
test("reconciliation compares Canvas JSON, geometry, and searchText without printing values", {
  ...
  try {
    ...
    const fields = changed.details.mismatches.map(
      (mismatch: { field: string }) => mismatch.field,
    );
