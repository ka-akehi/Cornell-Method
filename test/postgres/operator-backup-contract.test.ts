...
test("retention plan keeps daily and weekly slots and only prunes matching export files", () => {
  withTemporaryDirectory((directory) => {
    ...
    assert.equal(
      plan.kept.filter((entry: { reason: string }) => entry.reason === "daily").length,
      2,
    );
    assert.equal(
      plan.kept.filter((entry: { reason: string }) => entry.reason === "weekly").length,
      2,
    );
    assert.ok(
      plan.delete.some(
        (entry: { file: string }) => entry.file === "postgres-export-2026-07-10T12-00-00Z.dump",
      ),
    );
