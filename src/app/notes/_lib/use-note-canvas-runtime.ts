...
import {
  ...
  pointFromPointer,
} from "./canvas-editor-geometry";
import {
  isCanvasDrawingTarget,
  isCanvasShapeTextEditorTarget,
} from "./canvas-editor-document";
import {
  DEFAULT_FONT_FAMILY,
...
import { applyCanvasSurfaceDimensions } from "./canvas-surface";
import type {
  CanvasStyleChange,
  CanvasStyleDefaults,
  DragDraft,
...
function enableTextEditing(canvas: FabricCanvasLike) {
  canvas.getObjects().forEach((object) => {
    const metadata = object.get("canvasElement") as
      | { element?: { type?: string } }
      | undefined;
    if (metadata?.element?.type === "text") object.set({ editable: true });
  });
}
      ...
      try {
        ...
        const onMouseDown = (event: FabricEventLike) => {
          ...
          const isPreviewTarget = [
            event.target,
            ...(event.subTargets ?? []),
          ].some((target) => target?.get("isCanvasPreview") === true);
