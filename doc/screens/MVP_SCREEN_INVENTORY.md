...
| 基本情報 | create は `.note-paper-meta-grid`、detail は `MetaGrid`。label + control が縦積みで画像の一行帯ではない | 学習日・学習元・タグをコンパクトな inline/flex メタ情報帯として扱う。タイトルは基本情報の折りたたみ対象にしない |
| Cornell | create は wrapper 内 `min-w-[640px]` の `3fr / 7fr`、detail は `lg` 以上だけ `0.32fr / 0.68fr`。Cue は rounded row、横罫線は grid の上下中心 | 約 30% / 70% を維持し、本文 Canvas を 68〜72% の主列にする。Cue / Canvas を一枚の ruled surface とし、detail も局所横 scroll を持つ |
| Canvas 本文 | Canvas editor / viewer を本文列に置き、Cue と Summary を残す。Canvas JSON の保存・復元境界と `bodyMode` が存在する | 本文領域に幅・高さの数値入力と適用操作を置く。既定 1200x800、許容 320〜4000 の整数 px。Fit / 50% / 100% / 200% は表示倍率として分離し、page 寸法の選択肢にしない |
...
| 操作 | 明示保存、Cue / tag / review、field error、削除 confirm などの handler は現行コードにある。edit には外側 `閲覧へ戻る` と内側 `キャンセル` が重複 | 閲覧の編集・復習・削除は上部操作帯、作成・編集の保存・キャンセルは紙面 footer、復習済みは Summary 後へ集約。既存 handler / API payload は維持 |
...
| 基本情報 | 学習日・学習元・タグはコンパクトなメタ情報帯 | 大きな基本情報カードが本文開始位置を押し下げないか |
| Cornell | Cue 28〜32% / 本文 68〜72%、約 30% / 70% を基準。境界は縦罫線、Cue 行と本文行は薄い横罫線 | 1280 / 1440px で本文が Cue より広く、見出し・本文・箇条書きが読み物として見えるか |
| 本文表示 | Canvas 本文は自由配置面として表示し、既存 `bodyMode=markdown` のノートだけ Markdown Preview を使う | Canvas の用紙寸法と表示倍率が別 state か。用紙変更で要素の `x`, `y`, `width`, `height`, `points`, `style` が変わらないか |
...
- デスクトップ優先を維持する。1280px / 1440px 前後で深いクロームの中央に viewport 連動の広い紙面を置き、紙面が狭い固定幅にならず、Canvas 本文列に横幅を優先配分する。既存の親 `max-w-6xl` や固定 `1280px` cap による圧迫を再導入しない。
- 1280px / 1440px では Cue 約 30% / 本文約 70% を確認し、本文列内の Canvas 用紙、幅・高さ入力、適用操作が狭くならないことを確認する。Cue / Summary の Markdown Preview はそれぞれの入力欄に属する。
- 768px 未満は本格的な編集最適化を行わない。現行 MVP 方針に従い、Cornell 作業面の局所横スクロールは許容するが、ページ全体・`body`・アプリクロームの意図しない横 overflow は作らない。
- 375px では paper shell が viewport を超えず、紙面外周、基本情報帯、Summary が通常の縦スクロールで確認できる。Cornell wrapper 内だけの局所的な横スクロールを採用する場合も、Cue 追加・削除、Cue 入力、Canvas 操作、用紙サイズ入力、Summary Preview、保存・キャンセルへ到達できる。detail も create と同じ wrapper rule を使い、`lg` breakpoint だけで一列化しない。
...
| Canvas 本文 | 本文列に自由配置 Canvas と用紙サイズ操作を置く。legacy Markdown body mode のみ `Markdown Preview` を使う | Canvas editor / viewer と JSON 境界はあるが、可変用紙サイズの入力・適用と表示倍率の分離は未実装。legacy body mode は rendered body を使う | 部分一致 |
...
| --- | --- | --- | --- |
| create | `/notes/new` の一枚紙面。title input → metadata → Cue / Canvas 用紙・サイズ操作 → Summary → footer | `NoteEditor mode="create"`、Canvas editor / Summary Preview、save は明示操作 | inline metadata、ruled Cornell、Canvas paper surface / Summary footer / chrome state が不足 |
| detail view | `/notes/[id]` の同じ紙面。title h1 → metadata → Cue / Canvas viewer → full-width Summary | `NoteDetailModes mode="view"`、Canvas / Summary visible、上部 actions | shell はある。edit/review との wrapper 共通化、罫線、metadata one-line、Summary footer、chrome state が不足 |
| detail edit | view と同じ紙面の title 帯を一つの input に置換し、create と同じ Canvas / Summary / footer を使う | `note-detail-modes.tsx` 外側 h1 の下に `NoteEditor shell=false` が title input を再描画。戻る操作も二重 | title 重複を解消し、作成・編集の入力操作を維持したまま一枚の title band に統合 |
| review | view と同じ位置。Cue を手がかりにし、body → Summary の順に開示。復習記録は Summary 直後 | `showBody=false`、`showSummary=false`。本文表示後だけ Summary button enabled。完了後に body/Summary を閉じ view へ | 開示順は実装済み。detail の Cornell local wrapper と Summary/footer の見た目だけを補正し、初期 mask は変更しない |
...
- PNG に独立した説明欄が見えないことと、現行 MVP の payload に存在する項目を追加することを混同しない。現行のメタ情報は学習日・学習元・タグに限定する。
- PNG の Cue / 本文の見た目を理由に Canvas 要素を NoteCard へ分割したり、Cue と本文範囲の ID を追加したりしない。現行 MVP の Cue list / `CanvasDocumentV1` を維持し、legacy `bodyMode=markdown` は自動変換しない。
...
| `note-detail-modes.tsx` の閲覧・復習シェル、Canvas／Summary の表示状態配置 | Prisma schema、SQLite、`Notebook.bodyMode` / legacy `Notebook.body`、`NotebookCanvas`、Cue / Tag 関係 |
| `markdown-field.tsx` の Cue / Summary textarea / Preview 配置と共通 styling | 明示保存、Canvas JSON の保存領域、Cue / Tag 全置換、手動 `nextReviewDate`、復習 API の契約 |
...
| notes payload | `title`、`noteDate`、`sourceType`、`sourceTitle`、`bodyMode`、条件付きの `body` / `canvas`、`summary`、`nextReviewDate`、`cues[{ text, order }]`、`tags[{ name, color }]`。`bodyMode=canvas` の本文は `CanvasDocumentV1`、`bodyMode=markdown` は legacy Markdown 文字列 |
...
| Prisma / SQLite | MVP の `Notebook`、`NotebookCanvas`、`Tag`、`NotebookTag`、`Cue` と既存 column mapping を維持。用紙サイズは既存 Canvas JSON の `page.width` / `page.height` に保存し、新しい page 専用 column や migration は追加しない。`NoteCard`、`CueCard`、`NoteCueLink`、draft 用 model を追加しない |
...
| 3 | `UI-PAPER-013` | `src/shared/markdown/markdown-field.tsx` | `UI-PAPER-012` | Cue / Summary の textarea → Preview stacked と paper-native Preview surface を固定し、GFM / sanitize / checkbox read-only / label / empty state / 既存 prop 互換を維持 |
| 4 | `UI-PAPER-014` | `src/app/notes/_components/note-editor.tsx` | `UI-PAPER-012`, `UI-PAPER-013` | create/edit の一つの title band、metadata、Cue 行、Canvas editor と用紙サイズ操作、全幅 Summary、右下 save/cancel footer を実装。Cue/tag/date/validation、POST/PATCH、明示保存は不変 |
...
| 利用者 | ローカル環境で利用する個人ユーザー |
| 表示データ | タイトル、学習日、学習元、タグ、Cue リスト、Canvas 本文（legacy `bodyMode=markdown` は Markdown 表示）、Markdown 表示されたサマリー、次回復習日、最終復習日時、要約状態、復習モードの本文表示状態 |
...
| エラー / 空状態 / ローディング | 詳細取得中、404 ノートなし、API エラー、Canvas 復元エラー、legacy Markdown 表示エラー時のフォールバック、削除失敗、削除確認キャンセル。 |
| MVP 範囲 | 共通の詳細画面シェルによる Canvas 閲覧、編集モード切替、復習モード切替、確認ダイアログ付き削除、Cue / Summary の Markdown 表示と sanitize。復習モードでは共通 Cornell の Canvas 本文領域と Summary 内容を初期非表示にし、Cue で想起して本文を確認した後に Summary を開く。 |
...
| Data | `id`, `title`, `noteDate`, `sourceType`, `sourceTitle`, `bodyMode`, `body`, `canvas`, `summary`, `nextReviewDate`, `reviewedAt`, `cues`, `tags` |

#### SCR-002 UI State / Conditions

| 区分 | 内容 |
| --- | --- |
| 主な表示項目 | 共通 header のアプリ名・一覧・新規・状態バッジ、紙面上部のタイトル、タグ、学習日、学習元、次回復習日、最終復習日時、Cue、Canvas 本文または既存 Markdown 本文、全幅 Summary、一覧へ戻る、編集、復習、削除 |
| 共通シェル | 閲覧／復習で深いクローム中央の暖色一枚紙面を共有する。タイトル帯、コンパクトなメタ情報帯、Cornell（Cue 28〜32% / 本文 68〜72%、約 30% / 70%、中央の縦罫線）、全幅 Summary の順序と位置を固定する。セクションを細かいカードへ分割せず、薄い横罫線・余白・タイポグラフィで区切る。 |
| モード差分 | 閲覧は Canvas または既存 Markdown 本文と Summary を表示する。復習は同じ本文領域を初期マスクし、Summary 内容を初期非表示にする。本文確認後に Summary を開き、復習記録を Summary 後へ置く。モードラベルや操作ボタンの違いは紙面骨格の変更を意味しない。 |
...
| empty | タグなし、Cue なし、本文なし、サマリーなし、次回復習日なし、最終復習日時なしをそれぞれ明示する。 |
...
| 入力データ | タイトル、学習日、学習元タイプ、学習元タイトル、タグ、Cue リスト、Canvas 本文、用紙幅・高さ、Summary Markdown、次回復習日 |
| 主要アクション | Cue 追加、Cue 削除、タグ入力、Canvas 操作、用紙サイズ入力・適用、Summary 入力、保存、キャンセル |
| 副作用のある操作 | 作成保存で `POST /api/notes` を呼び出し Notebook / NotebookCanvas / Cue / Tag / NotebookTag を作成する。編集保存で `PATCH /api/notes/:id` を呼び出し Notebook / Canvas を更新し Cue / Tag 関連を全置換する。未登録タグは保存時に自動作成される。 |
...
| MVP 範囲 | 明示的な保存、Cue / Summary の Markdown preview、Canvas 本文、用紙サイズの数値入力・適用、Cue 追加・削除、タグ最大 12 件、タイトル・日付等のバリデーション、Cue / Tag は更新時全置換。新規作成の次回復習日は `noteDate + 7日` を初期値とし、変更・空欄化を許可する。 |
...
| Data | `title`, `noteDate`, `sourceType`, `sourceTitle`, `bodyMode`, `body`, `canvas.page.width`, `canvas.page.height`, `summary`, `nextReviewDate`, `cues[].text`, `cues[].order`, `tags[].name` |

#### SCR-003 UI State / Conditions

| 区分 | 内容 |
| --- | --- |
| 主な表示項目 | 共通 header の `新規作成` / `編集中` 状態、紙面上部のタイトル入力、コンパクトな学習日・学習元・タグのメタ情報帯、Cue リスト、Canvas 本文、幅・高さの数値入力と適用、表示倍率（用紙サイズとは別）、全幅 Summary textarea → Preview（開閉または簡易表示）、次回復習日、紙面フッターの保存・キャンセル |
| 目標レイアウト | タイトルを常時表示の主見出し位置とし、基本情報を大きなカードにしない。Cornell は Cue 28〜32% / Canvas 本文 68〜72% で本文を主役にし、中央の縦罫線と薄い横罫線で区切る。用紙サイズは幅・高さの数値入力で変更し、Fit / 50% / 100% / 200% は表示倍率として分離する。保存は Summary 後の右下主操作。 |
...
| empty | Cue 0 件は `Cue は未追加です。`。Canvas が空でも自由配置面を表示する。既存 Markdown mode の本文 preview 空は `本文のプレビューはまだありません。`。サマリー preview 空は `サマリーのプレビューはまだありません。`。 |
| 状態差分 | 作成・編集は入力可能、閲覧は表示専用、復習は本文マスクと Summary 初期非表示。紙面のタイトル→メタ情報→Cornell→Summary の骨格は変えない。 |
...
`SCR-004` は独立した画面ではなく、`SCR-002 Note Detail` の共通詳細画面シェル上で切り替える復習モードを表す。復習時も Cue、本文領域、サマリーの位置は閲覧モードと共通にし、本文領域と Summary 内容を初期非表示にする。Cue で想起し、本文を確認した後に Summary を開く。
...
紙面のタイトル常時表示、基本情報の圧縮、Cue / Canvas 本文の 30% / 70%、本文用紙のサイズ操作、Summary の位置、操作配置、375 / 768 / 1280 / 1440px の受け入れ境界は本書と [`MVP_SCREEN_DESIGN.md`](MVP_SCREEN_DESIGN.md) で決定済みです。後続 Worker はこれらを追加質問なしで、監査後の `UI-PAPER-011` から着手できます。
