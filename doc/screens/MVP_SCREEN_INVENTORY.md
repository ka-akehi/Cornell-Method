...
| MVP 範囲 | 共通の詳細画面シェルによる閲覧、編集モード切替、復習モード切替、確認ダイアログ付き削除、Markdown 表示、sanitize。復習モードでは共通 Cornell の本文領域だけを初期マスクする。 |
...
`SCR-004` は独立した画面ではなく、`SCR-002 Note Detail` の共通詳細画面シェル上で切り替える復習モードを表す。復習時も概要、Cue、本文領域、サマリーの位置は閲覧モードと共通にし、本文領域だけをマスクする。

| 項目 | 内容 |
| --- | --- |
| 画面ID | `SCR-004` |
| 画面名 | Review |
| 目的 | 共通の詳細画面シェルで Cue とサマリーを手がかりに本文を思い出し、復習済み状態を記録する。 |
| 利用者 | ローカル環境で利用する個人ユーザー |
| 表示データ | タイトル、学習日、学習元、タグ、概要、Cue リスト、サマリー、本文表示状態、次回復習日、最終復習日時 |
...
| MVP 範囲 | 共通シェル内の本文初期マスク、本文表示/非表示切替、復習済み更新、次回復習日の手動設定またはクリア。採点や正誤判定なし。 |
...
| 区分 | 内容 |
| --- | --- |
| 主な表示項目 | 共通シェルのタイトル・メタ情報・概要、Cue、本文領域、サマリー、本文非表示メッセージ、本文表示/非表示ボタン、次回復習日、復習済みにする、閲覧へ戻る |
| レイアウト | 閲覧モードと同じヘッダー、概要、Cue／本文の Cornell 配置、サマリーの順序と位置を維持する。本文表示状態と復習操作・記録だけがモード固有の差分である。 |
