...
- ノート本文を最も広い読み書き領域にし、タイトルと本文の視線を最短にする。
...
3. **概要の開閉領域**: 概要はメタ情報帯の下に短い表示領域として置く。閲覧・復習では内容があれば表示し、編集では `概要` の開閉操作から textarea を開く。新規作成で入力を促す必要がある場合は開いた状態で開始してよいが、常に大きな高さを確保しない。
4. **Cornell 本文領域**: Cue と本文を紙面の中心に置く。Cue は左、本文は右で、本文側を主役にする。
5. **Summary と紙面フッター**: Cornell の下に Summary を置き、次回復習日と保存・キャンセルなどの操作を紙面の末尾にまとめる。
...
| 状態 | タイトル／メタ情報 | 概要 | Cornell | Summary / フッター |
| --- | --- | --- | --- | --- |
| 作成 | タイトル入力を主見出し位置、学習日・学習元・タグを入力可能なメタ情報帯 | 入力を促すため開いた状態で開始してよい | Cue 入力リスト、本文 textarea → Preview を同じ本文列に縦配置 | Summary textarea と任意の簡易 Preview。次回復習日、キャンセル、保存を紙面下部に置き、保存を右下の主操作にする |
| 編集 | 作成と同じ位置に既存値を表示・編集 | 開閉可能。閉じても値は失わない | 作成と同じ。本文は 1 本の Markdown 文字列 | `PATCH` の明示保存。保存・キャンセルを右下のフッターに集約 |
| 閲覧 | タイトルを大きな見出し、メタ情報を表示専用帯として表示 | 内容があれば短く表示し、全文は開閉可能 | Cue リストを左、Markdown 本文を右。本文列を主役にする | Summary を全幅で Markdown 表示。上部に編集・復習・削除、下部に次回復習情報 |
| 復習 | 閲覧と同じ位置・値。状態バッジは `復習中` | 概要の位置を変えない | Cue を先に表示。本文列は初期マスクし、列幅を縮めない。本文確認後に開示 | Summary の内容は初期非表示。本文確認後に同じ全幅領域で開き、復習済み操作をその直後に置く |
...
- 本文は、長文 Markdown の入力・読み返し・Preview の確認に使うため最も広い列とします。本文を主役にする理由は、学習記録の情報量と読み書きの滞在時間が Cue より大きく、狭い本文では紙面中心という目的が失われるためです。
- 本文列の中では、**textarea と Markdown Preview を縦並びにすることを目標の基本配置**とします。本文列全体の幅を確保したうえで入力から Preview へ下に流すことで、入力幅と表示幅をともに維持します。本文内の入力／Preview 横分割は旧実装の状態であり、最初の目標実装では採用しません。
- 閲覧モードでは本文列に Markdown 表示だけを置き、編集モードでは本文 textarea の下に Preview を置きます。Preview は表示専用で、GFM と sanitize の契約を維持します。
...
- `375px` では紙面外周を縮めてページ全体の横幅に収め、タイトル・メタ情報・概要・Summary は通常の縦スクロールで到達できるようにする。Cue の追加・削除、Cue 入力、本文入力、Preview、保存・キャンセルも到達可能にする。
...
- すべての title、日付、学習元、タグ、概要、Cue、本文、Summary、次回復習日は label または明確な accessible name を持つ。エラーは `aria-invalid` と `aria-describedby` で対象入力に関連付ける。
...
| 本文入力 / Preview | create/edit の本文は `MarkdownField layout="stacked"` で textarea → Preview まで実装済み。Preview 自体は `MarkdownPreview` の角丸白カード。detail view/review は本文表示のみで、画像にある編集面の Preview treatment はない | 本文列の中で textarea → Markdown Preview を縦に並べる。Preview は本文列の下に同じ紙面の表示領域として置き、GFM / sanitize / checkbox 表示専用を維持する |
...
| 6 | 本文 Preview（本文列下部、y 約 619〜783） | 本文の下に `Markdown Preview` 見出し、淡い紙面色、上端の細い破線、本文の Markdown 表示 | 編集時は本文 textarea → Preview を同じ右列に縦配置する。閲覧時は同じ本文領域に表示専用 Markdown、復習時は同じ領域を mask → 表示する。GFM / sanitize / checkbox 表示専用は共通責務として維持する |
...
| --- | --- | --- | --- | --- |
| create | `/notes/new` → `src/app/notes/new/page.tsx` → `NoteEditor mode="create"` | `.note-paper-page` の中に `form.note-paper-editor.note-paper-shell.note-paper-content`。`.note-paper-heading` に `TitleInput`、`.note-paper-meta-grid`、概要 `<details>`、`.note-paper-cornell-grid`、Summary `MarkdownField`、`.note-paper-footer` | 概要は open。Cue 追加・削除、date/select/source/tag 入力、本文 textarea、本文 Preview、Summary textarea / Preview、次回復習日、キャンセル、明示保存が表示される。保存中は `保存中...` かつ disabled | 紙面・タイトル・metagrid・body stacked・footer の骨格は部分的に一致。右上 chrome state、画像相当の ruled line、inline meta、footer の orange primary は未達 |
| detail view | `/notes/[id]` → `NoteDetailModes mode="view"` | `.note-paper-shell.note-paper-content.note-paper-detail`。heading は mode kicker + `h1.note-paper-title` + 戻る / 編集 / 復習 / 削除。続いて `MetaGrid`、alert、概要 `<details>`、Cornell の Cue `Section` と body `Section`、Summary `Section` | 本文と Summary の Markdown は常時表示。概要は初期 closed、削除は confirm 後に実行 | 一枚紙面と title h1 / body width は部分一致。メタ情報は grid、Cue と body は nested Section/card、Summary は footer ではなく section、chrome state は未実装 |
| detail edit | `/notes/[id]` の `mode="edit"` → `NoteDetailModes` → `NoteEditor shell={false}` | 外側に `.note-paper-detail-editor` と mode 見出し `h1`。その直下に embedded `NoteEditor` が title `TextInput`、metagrid、概要、Cornell、Summary、footer を描画 | 外側の `閲覧へ戻る` と、内側 footer の `キャンセル` が両方存在。フォーム入力、Cue / tag / Markdown / 明示保存は維持 | 画像の一枚タイトルに対し title が外側 h1 + 内側 input の二重になる。create と同じ paper sequence へ統合する必要がある |
| review | `/notes/[id]` の `mode="review"` → `NoteDetailModes` | view と同じ `.note-paper-detail` / heading / MetaGrid / overview / Cornell shell。body は `showBody`、Summary は `showSummary`、最後に `復習記録` Section | mode 切替時に `showBody=false`、`showSummary=false`。Cue と概要は見え、本文 placeholder と `本文を表示`、Summary placeholder と disabled `本文確認後に開く` が表示。本文表示後に Summary を開き、`復習済みにする` で API 更新 | 本文・Summary の初期開示順は契約と一致し実装済み。画像の Summary が見えるのは `編集中` を示す create/edit 視覚正本と解釈する。review は同じ位置で内容だけ mask する |
...
| 本文 Preview の位置 | 本文列の下部に `Markdown Preview`。本文と同じ紙面面、淡い境界 | create/edit の body `MarkdownField` は stacked で Preview が本文列内にある。`MarkdownPreview` は白い rounded card。detail view/review は rendered body のみで、edit visual の preview treatment はない | 部分一致（create/edit の配置は実装済み） | shared Markdown component の paper-native preview class と空 state を整え、利用側で本文列の上下関係を固定する。GFM / sanitize / checkbox は維持 |
...
| 既存フォーム操作 | 画像に見えない入力・保存操作も壊さず使えること | 明示保存、POST/PATCH、Cue add/remove/order、タグ候補 / 新規タグ / max 12 / duplicate error、概要開閉、date max、field error、保存中 disabled、cancel、review body/Summary reveal、delete confirm が存在 | 実装済み（保護対象） | layout task では DOM の意味と event handler を保ち、visual change と state placement だけを行う |

### 画像と MVP 仕様が競合する点の解決

| 論点 | 画像だけを読んだ場合 | 既存 MVP 契約との競合 | 正本としての解決 |
| --- | --- | --- | --- |
| 画像の state | 右上が `編集中` で、本文 Preview と Summary が見えている | 閲覧・復習の状態も同じ画像で表そうとすると、復習時 Summary 初期非表示と衝突する | PNG は create/edit の visual reference と解釈する。view は body/Summary 表示、review は同じ位置で body/Summary 内容を初期 mask する |
| 概要 | 画像の title → metadata → Cornell の間に独立した概要が見えない | `overview` は既存 payload / form / detail display の MVP 項目 | 概要を削除・本文へ混ぜない。metadata の直下に短い `<details>` / disclosure として置き、大きな card にしない |
| Cue / 本文のデータ構造 | 画像は番号付き Cue 行と複数段落の本文を見せる | MVP は Cue リストと一つの Markdown `body`。NoteCard / D&D / Cue link は Phase 2 | 行・罫線・番号の視覚だけを寄せ、Cue / body のデータ分割や ID 追加は行わない |
...
| body Preview | body textarea → Preview を body 列内に縦配置。Preview の幅は body 列いっぱい | Preview は body 列の下へ到達可能。Markdown table / code の overflow は Preview 内だけに閉じる | create/edit の stacked 配置は実装済み。detail は rendered body のみで、shared Preview styling はカード中心 |
...
| 3 | `UI-PAPER-013` shared Markdown field | `src/shared/markdown/markdown-field.tsx` | body の textarea → Preview stacked 配置と paper-native Preview surface を固定する。GFM、`rehype-sanitize`、checkbox の read-only / `preventDefault`、label、`aria-describedby`、empty state、既存 `layout` prop の互換性を維持する | `UI-PAPER-012` |
| 4 | `UI-PAPER-014` create/edit paper | `src/app/notes/_components/note-editor.tsx` | create と embedded edit の title 重複をなくし、metadata inline/wrap、概要 disclosure、Cue 行、本文 Preview、全幅 Summary、次回復習日・cancel・orange primary save の footer を一つの順序へ揃える。Cue/tag/date/validation、POST/PATCH、明示保存は不変 | `UI-PAPER-012`, `UI-PAPER-013` |
...
- `src/shared/markdown/markdown-field.tsx` の本文入力と Preview の配置・共通 styling。GFM、`rehype-sanitize`、Preview checkbox の表示専用挙動は維持する。
...
- Prisma schema、SQLite のデータモデル、Notebook の 1 本の `body`、Cue リスト、Tag / NotebookTag の関係。
...
紙面中心 UI の実装後は、NTE-020 の作成・編集、NTE-030 の閲覧・復習について 375 / 768 / 1280 / 1440px を再取得し、本文幅、タイトル帯、概要の開閉、Summary の表示順、ページ全体の overflow、保存・復習操作を再確認します。旧 screenshot の PASS や task status だけで新レイアウトを PASS と判定しません。runtime を直接確認していない viewport / 状態は未実施のまま記録します。
...
- 概要はメタ情報帯の下の `概要` 開閉領域に置く。新規作成では入力しやすいよう初期展開してよいが、textarea は短い高さから開始し、紙面の本文開始位置を押し下げない。
- Cornell は概要の直後に置く。Cue を左 28〜32%、本文を右 68〜72% とし、約 30% / 70% を基準に本文へ幅を優先配分する。
- Cue 追加は Cue 見出しの横、Cue 削除は各 Cue 行の中に置く。Cue はカードを積むのではなく、薄い区切り線を持つ行／リストとして扱う。
- 本文列は `ノート本文` の textarea、その下の Markdown Preview の順に縦並びにする。本文内の textarea / Preview 横分割は目標 UI の標準配置にしない。
...
- タイトル、メタ情報帯、概要、Summary は通常のページ縦スクロールで確認できるようにする。メタ情報は画面幅に合わせて折り返す。
- Cornell は 2 列の関係を保つため局所的な横スクロールを許容する。横スクロールは Cornell 作業面の内側に閉じ込め、紙面全体やナビゲーションの意図しない横 overflow は発生させない。
- 375px 前後でも Cue 追加・削除、Cue 入力、本文 textarea、本文 Preview、キャンセル、保存へ到達できることを最低限の条件とする。モバイルでの入力効率や表示密度の最適化は後続課題とする。
...
閲覧モードと復習モードは、目的と操作が異なっても、同じ紙面シェルを維持する。モードラベルやモードごとの操作ボタンは変更してよいが、タイトル帯、メタ情報帯、概要、Cornell、Summary の順序・位置をモードごとに組み替えない。
...
- メタ情報帯の下に概要の短い表示領域を置く。概要が長い場合も本文の開始位置を不必要に押し下げない。必要なら `概要を開く` / `概要を閉じる` で全文を確認できる。
- 概要の下に Cornell を置く。デスクトップでは Cue を左 28〜32%、本文を右 68〜72% とし、中央の縦罫線で境界を示す。約 30% / 70% の契約を保ちながら本文を主役にする。
- Cornell の下に紙面全幅の Summary を置く。閲覧では本文と Summary を Markdown 表示し、復習では同じ本文領域を初期マスク、同じ Summary 領域を初期非表示にする。
- 閲覧モードの編集・復習・削除は紙面上部の操作帯に置く。編集モードの保存・キャンセルは Summary 後の紙面フッターに置く。復習済み操作は Summary 後の復習記録領域に置く。
- モバイルでも「タイトル帯・メタ情報・概要 → Cornell → Summary」の基本順序を維持する。768px 未満では Cornell 内の局所横スクロールを許容するが、ページ全体の横 overflow は許容しない。
...
- ノート作成画面と同じ紙面順序・入力項目
- タイトルは紙面上部に常時表示し、概要は開閉領域、本文は Cue 右側の広い列、Summary は紙面下部に置く
- 本文 textarea と Preview は縦並びを基本とし、Summary Preview は開閉または簡易表示にする
...
- 1280px / 1440px 前後で、Cornell の Cue が約 30%、本文が約 70% となり、本文列が Cue より広い。本文の textarea が画面幅のために狭くなっていない。
- 1280px / 1440px 前後で、本文 textarea と Markdown Preview が本文列内で縦並びになり、本文 Preview の確認時にも本文の入力幅を確保できる。
- 基本情報が大きなカードとして本文を押し下げず、概要は開閉または短い表示領域として扱える。タグが長くてもメタ情報帯からはみ出さない。
...
- 375px 前後でページ全体の意図しない横 overflow がなく、基本情報と Summary は縦スクロールで確認できる。Cornell 内の局所横スクロールを許容する場合も、Cue 追加・削除、本文入力、Preview、保存へ到達できる。
- 375px / 768px で Markdown Preview の GFM、sanitize、checkbox の表示専用挙動が維持される。
