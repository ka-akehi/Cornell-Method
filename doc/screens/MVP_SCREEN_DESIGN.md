...
| 項目 | 内容 |
| --- | --- |
| 画面ヘッダー | `h1` の「ノート一覧」と `/notes/new` へ進む「新規作成」。冗長な補助文は置かない |
...
| --- | --- |
| 新規作成 | `/notes/new` へ遷移 |
| フリーワード入力 | 入力停止から 300ms 後に、他の最新条件を含めた検索を開始する |
| フリーワード入力中の Enter | pending debounce を取り消し、最新条件を待たずに適用する |
| From / To 変更 | 最新 query を含む全条件を直ちに適用する。有効な範囲だけ API request を開始する |
| タグ追加・削除 | 最新 query を含む全条件を直ちに適用する |
| 復習対象のみ toggle | 押下状態を切り替え、最新 query を含む全条件を直ちに適用する。native button として keyboard activation でき、visible な `ON` / `OFF` badge は表示しない |
| クリア | pending debounce を取り消し、query、From / To、タグ、タグ追加候補、review toggle を初期化して直ちに全件条件を適用する |
| ノート選択 | `/notes/[id]` へ遷移 |

visible な `検索` button は置かない。desktop では review toggle をタグ操作行の右側上端へ固定し、選択済みタグチップが複数行へ増えても toggle の縦位置を下げない。
...
| 対象 | 表示タイミング | 表示場所 | 文言 |
| --- | --- | --- | --- |
| 日付 From / To | From / To 変更時のライブ検索適用前、および From / To のフォーカスアウト時。Enter、タグ、review toggle など別条件から即時適用する場合も現在の日付範囲を検証する | 検索フォーム下 | `開始日は終了日以前の日付を指定してください。` |
| API query validation | 一覧取得 API が 400 を返した時 | 検索フォーム下または検索結果上の error alert | API の `message` を表示。field 別詳細が必要な場合は日付欄近くに表示する |

MVP の一覧画面では、日付は `<input type="date">` を前提にする。From / To 変更時は有効な範囲なら直ちに一覧取得し、From > To は request 前に拒否する。blur は現在の日付範囲の validation 契機の一つであり、blur 自体では検索を開始しない。`react-day-picker`、クイックセレクト、日付ソート切替は MVP 外とする。
...
| 復習対象のみ toggle | 原則 disabled にしない |
...
| 状態 | 表示 |
| --- | --- |
| 一覧取得中 | 検索結果エリアを `読み込み中...` に切り替える。visible な `検索` button は存在しない |
...
- ライブ検索成功時は検索結果、総件数、必要に応じてページャを更新する。
- query は 300ms debounce、Enter は即時、日付・タグ・review toggle は変更時に即時適用する。いずれもその時点の最新条件をまとめて使う。
- クリア時は pending query debounce を取り消し、全検索条件と review toggle を初期化して即時に全件条件へ戻す。
...
- 375px 前後でも Cue 追加・削除、Cue 入力、Canvas 操作、用紙サイズ入力、Summary Preview、キャンセル、保存へ到達できることを受け入れ条件とする。モバイルでの入力効率や表示密度の最適化は後続課題とする。
...
- タイトル直下に学習日、学習元、タグ、次回復習日、最終復習日時をコンパクトなメタ情報帯として置く。メタ情報を本文より大きなカードにしない。復習モードで保存済みの次回復習日を残す場合は「保存済み」と示し、復習記録の次回復習日入力と区別する。
...
| 復習済みにする | `POST /api/notes/:id/review` を呼び、`reviewedAt` を現在日時に更新。入力した `nextReviewDate` または null を保存し、成功後は API 応答値を画面へ反映 |
| 閲覧へ戻る | 閲覧モードへ戻る |
