...
3. `Settings`（shell / bridge / entrypoint / General と Data and Backup の操作は実装済み。実 provider / packaged runtime は未検証）: General（テーマ設定と manual update check bridge を含む）と Data and Backup（native file dialog / typed bridge による export・restore・削除）の入口と、現行 `/backup` からの段階移行。
...
7. `バックアップ / 復元`: 手動 SQLite 書き出し、管理用バックアップと外部ファイルの別入口、検証、アトミックな切り替え、再オープン。外部 SQLite export は create-only / no-replace とし、既存 destination は `destination-exists` で拒否する。managed catalog metadata の `recoveryOnly` が true の restore 前 safety backup（`restore-<operationId>.sqlite.bak` 等）は物理ファイル・内部 catalog に保持して startup recovery / rollback に使い、Settings の通常復元一覧から除外する。通常一覧は user backup の最新 1 件だけとする。
