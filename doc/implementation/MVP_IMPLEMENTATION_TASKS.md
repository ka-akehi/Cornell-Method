...
フルリニューアル版 Cornell Method Notebook の MVP 実装を Worker タスクに分割した計画です。
...
実装順序を「UI先行」と「DB/API先行」のどちらにするかを決める。
...
- タイトル、学習日、学習元、タグ、Cue、本文、サマリー、次回復習日を入力できるようにする。
- Cue 追加・削除を実装する。
- 保存後に詳細画面へ遷移する。
...
発注者がこのタスク分割を承認した後、Manager は `codex-queue` へ最初の Worker タスクを投入する。
