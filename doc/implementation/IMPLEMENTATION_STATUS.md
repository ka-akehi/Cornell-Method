...
外部 SQLite export は create-only / no-replace である。既存 regular file は `destination-exists` として拒否し、`replaceExisting` / `allowReplaceExisting` permission と既存 destination への通常 rename publish は提供しない。検証後の inode 差し替えで race winner を失う P1 はこの境界で解消済みであり、focused tests 17/17 と再レビュー `APPROVE` を得ている。
