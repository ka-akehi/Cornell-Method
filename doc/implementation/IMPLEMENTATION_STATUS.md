...
2026-07-25 に修正後 runtime を再試行したが、Browser backend は `[]`、local server は `listen EPERM`、standalone Playwright Chromium は macOS MachPort permission error で起動できなかった。したがって unknown target の pointer / 保存 JSON は未確認のままで、`CANVAS-INTERACTION-001` の判定は変更しない。詳細は `summary/20260725/verify-canvas-unknown-target-pen-gesture-runtime-20260725-summary.md` を参照する。
