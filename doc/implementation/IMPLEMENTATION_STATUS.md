...
現在のコードは、`Notebook` に 1 本の Markdown 本文を保存し、`Cue` リスト、タグ、一覧検索、詳細画面内の閲覧・編集・復習、手動バックアップを提供する小さな MVP である。

現行 MVP と照合した重要な差分は次のとおり。

- 新規作成時の `nextReviewDate = noteDate + 7日` は実装されていない。新規フォームは空欄で始まり、空欄のまま保存すると `null` になる。
- 復習モードの本文は初期非表示になるが、Summary は常時表示される。契約が求める Summary の初期非表示は未達である。
- 概要は保存されるが、編集画面は通常の textarea、詳細画面は通常のテキスト表示であり、本文・Summary と同じ Markdown preview / sanitize 表示ではない。
...
| `/notes/new` | 明示保存の新規作成フォーム | `src/app/notes/new/page.tsx`, `src/app/notes/_components/note-editor.tsx` |
| `/notes/[id]` | 閲覧・編集・復習モード、確認後削除 | `src/app/notes/[id]/page.tsx`, `src/app/notes/_components/note-detail-modes.tsx` |
...
| Method | URL | 実装状況 | 根拠 |
| --- | --- | --- | --- |
| `GET` | `/api/notes` | 一覧・検索・ページング | `src/app/api/notes/route.ts`, `src/server/notes/infrastructure/read.repository.ts` |
| `POST` | `/api/notes` | ノート作成、Cue・タグ関連作成 | `src/app/api/notes/route.ts`, `src/server/notes/infrastructure/command.repository.ts` |
| `GET` | `/api/notes/:id` | ノート詳細取得 | `src/app/api/notes/[id]/route.ts` |
| `PATCH` | `/api/notes/:id` | ノート全体の明示更新、Cue・タグ関連の全置換 | `src/app/api/notes/[id]/route.ts`, `src/server/notes/infrastructure/command.repository.ts` |
...
現行 schema に存在する model は `Notebook`、`Tag`、`NotebookTag`、`Cue` の 4 つである。根拠は `prisma/schema.prisma`。

| Model | 実装上の責務・主な field |
| --- | --- |
| `Notebook` | `id`, `title`, `noteDate`, `sourceType`, `sourceTitle`, `overview`, `body`, `summary`, `nextReviewDate`, `reviewedAt`, `createdAt`, `updatedAt`, `deletedAt`。`body` が 1 本の本文で、復習情報は `nextReviewDate` / `reviewedAt`。 |
...
| ノート CRUD | タイトル、学習日、学習元、概要、本文、Summary、復習日を保存・取得・更新・削除。 | `src/app/api/notes/route.ts`, `src/app/api/notes/[id]/route.ts`, `src/server/notes/infrastructure/command.repository.ts` |
...
| 一覧検索 | タイトル・概要・本文・Summary・Cue の部分一致、日付範囲、タグ、`reviewDue`、ページング、空状態・loading・error 表示。 | `src/app/notes/_components/notes-list.tsx`, `src/server/notes/infrastructure/read.repository.ts` |
| 詳細モード | `/notes/[id]` 内で閲覧・編集・復習を切り替える。復習時は本文を隠す／表示する操作がある。 | `src/app/notes/_components/note-detail-modes.tsx` |
| Markdown 表示 | 本文・Summary の textarea と preview、GFM、sanitize、preview checkbox の表示専用化。 | `src/shared/markdown/markdown-field.tsx`, `package.json` |
...
| 契約項目 | 実際の挙動 | 判定 | 根拠 |
| --- | --- | --- | --- |
| 復習モードの本文・Summary | 本文は復習開始時に非表示で、ボタンで表示・再非表示できる。Summary は mode に関係なく常時 `MarkdownPreview` を描画する。 | 部分実装 | `src/app/notes/_components/note-detail-modes.tsx:124-149`, `src/app/notes/_components/note-detail-modes.tsx:320-361` |
...
| 概要の Markdown | 概要は保存されるが、編集は `TextArea`、詳細は通常の `<p>` 表示で、本文・Summary と同じ Markdown preview / sanitize を通らない。 | 部分実装 | `src/app/notes/_components/note-editor.tsx:211-218`, `src/app/notes/_components/note-detail-modes.tsx:306-314`, `src/shared/markdown/markdown-field.tsx` |
