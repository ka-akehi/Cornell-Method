...
| CANVAS-GESTURE-001 | line / arrow / rect / ellipse のクリック・ダブルクリック no-op、4px drag threshold、standalone text・shape inline text・shape drag の gesture 分離 | `/notes/new`（編集） | Browser runtime、1280 x 900、pointer | 2026-07-24 | 4 tool の click / double-click / 3px / 5px を一時ノートごとに実行。0 件 / 1 件の保存応答と type を比較。同じ tool の inline editor overlay 上の drag は新規要素を作らないことを確認。厳密な 4px、別 tool 切り替え後の分離、再読込は未確認 | 部分実施 | `summary/20260724/canvas-gesture-browser-qa-runtime-20260724.md`、`summary/20260722/canvas-browser-qa-partial-20260722.md`、`doc/designs/CANVAS_TOOLBAR_DESIGN.md` §9.2、`src/modules/notes/ui/hooks/use-note-canvas-runtime.ts`、`src/modules/notes/lib/canvas-editor-geometry.ts` |
