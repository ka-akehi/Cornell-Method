...
| 未確認 | 厳密な 4px 境界、preview / inline editor overlay / unknown metadata の遮断、shape inline text と shape drag の分離、保存後の再読込比較 |

この追補により空白および既存要素上の作成と click / double-click / 3px / 5px の境界は確認できたが、残る境界条件があるため `CANVAS-INTERACTION-001` と `CANVAS-GESTURE-001` は `部分実施` のままとする。
...
| CANVAS-INTERACTION-001 | 空白および既存の pen stroke、line、arrow、rect、ellipse、standalone text 上からの新規作成、preview / inline editor overlay / metadata 欠落 object の gesture 遮断 | `/notes/new`（編集） | Browser runtime、1280 x 900、pointer | 2026-07-24 | 一時ノートに基準 6 要素と重ね描き 6 要素を作成。保存応答を確認後に削除し、`query=Gesture` の残存 0 件を確認。preview / overlay / unknown metadata は未確認 | 部分実施 | `summary/20260724/canvas-gesture-browser-qa-runtime-20260724.md`、`summary/20260719/2143-sync-canvas-interaction-design-contract-20260719-5b6bd3a6-summary.md`、`src/modules/notes/ui/hooks/use-note-canvas-runtime.ts`、`src/shared/canvas/adapters/fabric/fabric-metadata.ts` |
| CANVAS-GESTURE-001 | line / arrow / rect / ellipse のクリック・ダブルクリック no-op、4px drag threshold、standalone text・shape inline text・shape drag の gesture 分離 | `/notes/new`（編集） | Browser runtime、1280 x 900、pointer | 2026-07-24 | 4 tool の click / double-click / 3px / 5px を一時ノートごとに実行。0 件 / 1 件の保存応答と type を比較。厳密な 4px、shape inline text、再読込は未確認 | 部分実施 | `summary/20260724/canvas-gesture-browser-qa-runtime-20260724.md`、`summary/20260722/canvas-browser-qa-partial-20260722.md`、`doc/designs/CANVAS_TOOLBAR_DESIGN.md` §9.2、`src/modules/notes/ui/hooks/use-note-canvas-runtime.ts`、`src/modules/notes/lib/canvas-editor-geometry.ts` |
