...
async function runAll() {
  for (const name of ["validate", "prepare", "build", "smoke", "runtime-http", "lifecycle", "package", "evidence"]) {
    const result = await runCommand(name);
    if (name === "validate" && result?.status !== "PASS") {
      throw new Error("baseline validation が PASS ではないため、後続の install/build/packaging を実行しません");
    }
  }
}
...
async function main() {
  ...
  if (name === "help") {
    console.log("Usage: npm run poc:<validate|prepare|build|smoke|lifecycle|package|evidence|all>");
    return;
  }
  if (name === "all") {
    await runAll();
