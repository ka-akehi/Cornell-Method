...
function buildDescendantClosure(records, rootPid) {
  if (!isPositivePid(rootPid)) throw new TypeError(`root PID must be a positive integer: ${rootPid}`);
  const byPid = new Map();
  ...
  while (queue.length > 0) {
    ...
    processes.push(toProcessEvidence(record, {
      depth: current.depth,
      relation: current.depth === 0 ? "sidecar-root" : "sidecar-descendant",
    }));
...
function observeDescendantClosure(rootPid, {
  ...
  try {
    ...
    if (expectedProcessGroupId != null) {
      if (!closure.rootObserved) scopeReasons.push("sidecar root が観測できません");
      else if (closure.rootProcessGroupId !== expectedProcessGroupId) scopeReasons.push("sidecar root の process group ID が expected group ID と一致しません");
      if (processTreeMembersOutsideExpectedGroup.length > 0) scopeReasons.push(`sidecar descendant closure に expected group 外の process があります: ${processTreeMembersOutsideExpectedGroup.map((record) => record.pid).join(", ")}`);
      if (groupMembersOutsideClosure.length > 0) scopeReasons.push(`expected group に sidecar descendant closure 外の process があります: ${groupMembersOutsideClosure.map((record) => record.pid).join(", ")}`);
    }
    const processGroupScoped = expectedProcessGroupId == null
      ? null
      : closure.rootObserved && closure.rootProcessGroupId === expectedProcessGroupId &&
        processTreeMembersOutsideExpectedGroup.length === 0 && groupMembersOutsideClosure.length === 0;
    return {
      ...
      expectedProcessGroupId,
      processGroupMatches: expectedProcessGroupId == null || !closure.rootObserved ? null : closure.rootProcessGroupId === expectedProcessGroupId,
      processGroupScoped,
      processGroupScopeReason: scopeReasons.length > 0 ? scopeReasons.join("; ") : null,
...
function canUseProcessGroupSignal(processGroup, observation) {
  return Boolean(
    ...
    Array.isArray(observation.pids) && observation.pids.length > 0 &&
    Array.isArray(observation.processTree) && observation.processTree.length > 0 &&
...
function sameProcessIdentity(before, after) {
  ...
  const afterGroup = after.processGroupId ?? after.pgid;
  if (before.processGroupId !== afterGroup) return false;
  if (before.commandName && after.commandName && before.commandName !== after.commandName) return false;
  return true;
}

function observeRemainingProcesses(beforeObservation, { readProcessTableImpl = readProcessTable } = {}) {
  const observedAt = new Date().toISOString();
  ...
  if (!beforeObservation?.rootObserved || !Array.isArray(beforeProcesses) || beforeProcesses.length === 0) {
    return {
      ...
      remainingPids: null,
      observedAt,
      reason: "shutdown 前の sidecar descendant closure が観測できていません",
    };
  }
  try {
    const table = readProcessTableImpl();
    ...
    for (const before of beforeProcesses) {
      ...
      if (sameProcessIdentity(before, current)) {
        remaining.push(toProcessEvidence(current, { depth: before.depth, relation: before.relation, shutdownBeforeParentPid: before.parentPid }));
      } else {
        identityMismatches.push({
          pid: before.pid,
          before: { commandName: before.commandName, processGroupId: before.processGroupId },
          after: { commandName: current.commandName, processGroupId: current.pgid },
        });
      }
    }
...
function waitForProcessTreeExit(beforeObservation, timeoutMs, { readProcessTableImpl = readProcessTable, intervalMs = 100 } = {}) {
  ...
  return new Promise((resolve) => {
    const poll = () => {
      ...
      if (after.status === "PASS" && after.remainingPids.length === 0) {
        resolve({ status: "PASS", timedOut: false, timeoutMs, elapsedMs: Date.now() - startedAt, after });
        return;
      }
      if (Date.now() >= deadline) {
        resolve({ status: after.status === "UNVERIFIED" ? "UNVERIFIED" : "TIMEOUT", timedOut: true, observationTimedOut: after.status === "UNVERIFIED", timeoutMs, elapsedMs: Date.now() - startedAt, after });
        return;
      }
...
function signalProcessTree(observation, signal, { sendSignal = process.kill } = {}) {
  ...
  for (const record of records) if (isPositivePid(record.pid) && !unique.has(record.pid)) unique.set(record.pid, record);
  const targets = [...unique.values()].sort((left, right) => (right.depth ?? 0) !== (left.depth ?? 0) ? (right.depth ?? 0) - (left.depth ?? 0) : right.pid - left.pid);
  const result = { status: "PASS", requested: targets.length > 0, method: EXPLICIT_PID_SIGNAL_METHOD, signal, targetPids: targets.map((record) => record.pid), sentPids: [], delivered: false, alreadyExitedPids: [], failedPids: [], errors: [] };
  if (targets.length === 0) {
    result.status = "UNVERIFIED";
    result.reason = "観測済みの sidecar process PID がありません。広範囲な kill は行いません";
    return result;
  }
...
function validateDedicatedProcessGroup(groupId, { sendSignal = process.kill, platform = process.platform, rootPid = groupId } = {}) {
  const result = { status: "UNVERIFIED", rootPid, groupId, method: PROCESS_GROUP_SIGNAL_METHOD, validationMethod: "detached child group leader plus process.kill(-groupId, 0)", validated: false };
  if (platform === "win32") {
    result.reason = "Windows では負の process group signal を使用しません";
    return result;
  }
  if (!isPositivePid(rootPid) || !isPositivePid(groupId)) {
    result.reason = `sidecar root PID / process group ID must be positive integers: root=${rootPid}, group=${groupId}`;
    return result;
  }
  if (rootPid !== groupId) {
    result.reason = "detached sidecar の root PID と process group ID が一致しません";
    return result;
  }
  try {
...
function signalProcessGroup(groupId, signal, { sendSignal = process.kill, observedPids = [] } = {}) {
  const result = { status: "PASS", requested: true, method: PROCESS_GROUP_SIGNAL_METHOD, signal, groupId, target: `-${groupId}`, observedTargetPids: observedPids, delivered: false, alreadyExited: false, error: null };
  try {
...
function assessRuntimeCleanup(shutdownEvidence) {
  const evidence = shutdownEvidence?.processTreeBeforeShutdown ? shutdownEvidence : shutdownEvidence?.runtime;
  ...
  const status = evidence?.status === "PASS" && isPositivePid(evidence?.runtimeRootPid) && before?.observationStatus === "PASS" && after?.observationStatus === "PASS" && Array.isArray(remainingPids) && remainingPids.length === 0 ? "PASS" : "FAIL";
  return {
    status,
    rootPid: evidence?.runtimeRootPid ?? null,
    observedBeforePids: Array.isArray(before?.pids) ? before.pids : null,
    remainingPids,
    beforeObservationStatus: before?.observationStatus ?? "UNVERIFIED",
    afterObservationStatus: after?.observationStatus ?? "UNVERIFIED",
    reason: status === "PASS" ? null : "sidecar root と shutdown 前に観測した descendant closure 全体の終了を確認できません",
  };
}
