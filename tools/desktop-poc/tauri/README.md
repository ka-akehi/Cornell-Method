...
This directory is an isolated, unselected Tauri candidate for the Cornell Method Notebook Desktop PoC. It does not change the root Next.js application or the Electron candidate. The final shell choice remains a separate owner decision after both candidates have comparable evidence.
...
The Rust manifest pins `tauri` and `tauri-build` to exact versions, but this repository does not carry a fabricated `Cargo.lock`. If Cargo cannot resolve those crates, or `cargo tauri` is absent, the affected build/package axis remains BLOCKED / UNVERIFIED and no placeholder lockfile, app, DMG, or measurement is produced.
